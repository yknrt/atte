...
class AttendanceController extends Controller
{
    ...
    public function attendanceUsers(Request $request)
    {
        $user = Auth::user();
        $registers = User::all();
        ...
        $times = Time::where('user_id', $user)->get();
        $dates = array();
        $rows = array();
        foreach ($times as $time) {
        ...
        $paginatedItems = $this->paginated_items($rows);
        return view('user', ['date' => $date, 'items' => $paginatedItems]);
    }
